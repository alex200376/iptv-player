...
function ChannelList() {
  ...
  const handleCheck = useCallback(async (ch: any) => {
  ...
  if (filteredGroups.length === 0) {
    return (
      <div className="px-3 py-8 text-center text-tv-xs text-tv-text-secondary">
        {useStore.getState().searchQuery ? '未找到匹配频道' : useStore.getState().activePlaylistId ? '该播放列表暂无频道' : '暂无频道，请先导入 M3U 文件'}
      </div>
    )
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between px-2 py-1 border-b border-tv-border">
        <span className="text-tv-xs text-tv-text-secondary">
          {filteredGroups.reduce((s, g) => s + g.channels.length, 0)} 频道
        </span>
        <button
          ...
          <Accordion.Item key={i} value={`group-${i}`}>
            <Accordion.Header>
              <Accordion.Trigger className="flex items-center gap-2 w-full px-3 py-1.5 text-tv-xs text-tv-text-secondary hover:bg-tv-bg-surface transition-colors group focus-visible:outline focus-visible:outline-2 focus-visible:outline-tv-focus-ring">
                ...
                <span className="font-medium text-tv-text-primary truncate text-tv-sm">{group.name}</span>
      ...
      {ctxMenu && (
        <ContextMenu
          ...
          items={[
            ...
            { label: favoriteIds.includes(ctxMenu.channel.id) ? '取消收藏' : '收藏', onClick: () => toggleFavorite(ctxMenu.channel.id), icon: <StarIcon filled={favoriteIds.includes(ctxMenu.channel.id)} /> },
...
function StatusDot({ status }: { status?: string }) {
  ...
  return (
    <span className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${colors[status || 'unknown'] || colors.unknown}`} />
  )
}
  ...
  return (
    ...
    >
      ...
      <span
        ...
        title={isFav ? '取消收藏' : '收藏'}
      >
        <svg className="w-3.5 h-3.5" viewBox="0 0 15 15" fill={isFav ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="1.5">
